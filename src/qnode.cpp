...
#include <sstream>
#include "../include/gui_drok5/qnode.hpp"
...
namespace gui_drok5 {
    ...
    *****************************************************************************/
    ...
    bool QNode::init() {
        ros::init(init_argc, init_argv, "gui_drok5");
        if ( ! ros::master::check() ) {
            return false;
        }
        ...
        *****************************************************************************/
        ...
        Front_RawImage_subscriber = n.subscribe("/usb_cam/image_raw/compressed", 1000, &QNode::Front_RawImage_Callback, this);
        Front_YoloImage_subscriber = n.subscribe("/usb_cam/image_raw2/compressed", 1000, &QNode::Front_RawImage_Callback, this);
        SideImage_subscriber = n.subscribe("/usb_cam/image_raw/compressed", 1000, &QNode::SideImage_Callback, this);
        Web_vd_server_state_subscriber = n.subscribe("web_vd_server_state", 1000, &QNode::Web_vd_server_state_Callback, this);
        Velodyne_Lidar_state_subscriber = n.subscribe("velodyne_state", 1000, &QNode::Velodyne_Lidar_state_Callback, this);
        Detect_KorOrNor_subscriber = n.subscribe("detected_class", 1000, &QNode::Detect_KorOrNor_Callback, this);
        Autodriving_state_subscriber = n.subscribe("autodriving_state", 1000, &QNode::Autodriving_state_Callback, this);

        // Sc
        Screenshot_subscriber = n.subscribe("/screenshot/image_raw", 1000, &QNode::Screenshot_Callback, this);

        // Battery
        BatteryFloat_subscriber = n.subscribe("/remaining_amount", 100, &QNode::BatteryFloat_Callback, this);
    ...
    *****************************************************************************/
    ...
    void QNode::Front_RawImage_Callback(const sensor_msgs::CompressedImage::ConstPtr& front_img) {
        State[5] = 1;
        cv::Mat image;
        try {
    ...
    void QNode::Front_YoloImage_Callback(const sensor_msgs::CompressedImage::ConstPtr& front_img) {
        State[6] = 1;
        cv::Mat image;
        try {
            image = cv::imdecode(cv::Mat(front_img->data), 1);
    ...
    void QNode::SideImage_Callback(const sensor_msgs::CompressedImage::ConstPtr& front_img) {
        State[7] = 1;
        cv::Mat image;
        try {
            image = cv::imdecode(cv::Mat(front_img->data), 1);
        ...
        int frame_width = 640;
        int frame_height = 480;
        qt_side_image = QImage((const unsigned char*)(frame.data), frame.cols, frame.rows, QImage::Format_RGB888).scaled\
            (frame_width, frame_height, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        Q_EMIT statusUpdated();
    }

    void QNode::Web_vd_server_state_Callback(const std_msgs::UInt16& state_msg) {
        State[8] = state_msg.data;
        Q_EMIT statusUpdated();
    }

    // Velodyne pkg들을 실행시켰을 때 받는 신호
    void QNode::Velodyne_Lidar_state_Callback(const std_msgs::UInt16& state_msg) {
        State[9] = state_msg.data;
        //Q_EMIT velodyneUpdated();
        Q_EMIT statusUpdated();
    }

    void QNode::Detect_KorOrNor_Callback(const std_msgs::String::ConstPtr& detect_msg) {
        if (detect_msg->data == "NOR")
            State[10] = 1;
        else
            State[10] = 0;
        Q_EMIT statusUpdated();
    ...
    void QNode::Autodriving_state_Callback(const std_msgs::UInt16& state_msg) {
        State[11] = state_msg.data;
        Q_EMIT statusUpdated();
    }
    ...
    void QNode::BatteryFloat_Callback(const std_msgs::Float32& battery_msg) {
        nBattery = (int)(battery_msg.data * 100);
